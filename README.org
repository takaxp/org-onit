...
- visiting a heading if =org-onit-include-no-status-heading= is non-nil and the task is not ~DONE~
...
- switching to other headings if the task has a todo status (e.g. ~TODO~)
- switching to other headings if =org-onit-include-no-status-heading= is non-nil and the task is not ~DONE~
- making the task ~DONE~
- calling =org-onit-toggle-doing=
...
** Options
 ...
 - org-onit-use-unfold-as-doing
   - Call =org-clock-in= when a heading is unfold and not clocking.
   - Default: nil
